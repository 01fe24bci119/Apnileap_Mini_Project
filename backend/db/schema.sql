...
-- ============================================================
-- Themes (Department -> Theme -> Projects)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS themes (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT uq_dept_theme UNIQUE(department_id, name)
);

ALTER TABLE projects ADD COLUMN IF NOT EXISTS theme_id UUID REFERENCES themes(id) ON DELETE SET NULL;
...
BEGIN
    FOR t IN
        SELECT unnest(ARRAY['users','institutes','departments','student_teams','projects','project_students',
                             'milestones','kpis','issues','corrective_actions','support_requests','themes','workspace_tasks'])
    LOOP
        EXECUTE format(
