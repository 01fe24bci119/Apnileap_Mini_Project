...
async function createAction(req, res, next) {
    try {
        ...
        if (!issueId) {
            return res.status(400).json({ error: 'A corrective action must be associated with an existing challenge.' });
        }
        const { rows: issueRows } = await pool.query(
            `SELECT id FROM issues WHERE id = $1 AND project_id = $2`,
            [issueId, req.params.projectId]
        );
        if (!issueRows.length) {
            return res.status(404).json({ error: 'The specified challenge was not found in this project.' });
        }

        const { rows } = await pool.query(
            `INSERT INTO corrective_actions (project_id, issue_id, description, owner_name, due_date, evidence)
             VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
            [req.params.projectId, issueId, description.trim(), ownerName, dueDate || null, evidence || null]
        );
