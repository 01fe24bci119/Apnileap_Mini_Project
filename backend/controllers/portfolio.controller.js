...
// mirroring canAccessProject(): global roles see everything; mentors see only
// their projects; students see only their team's projects; everyone else sees
// projects in their institutes or departments.
function scopeClause(user, startIndex) {
    ...
    if (isFacultyOnly(user)) {
        return {
            sql: `(p.mentor_user_id = $${startIndex} OR lower(trim(p.faculty_mentor_name)) = lower(trim($${startIndex + 1})) OR p.id = ANY($${startIndex + 2}::uuid[]))`,
            params: [user.id, user.fullName || '', a.projectIds],
        };
    }
    if (isStudentOnly(user)) {
        return {
            sql: `p.id = ANY($${startIndex}::uuid[])`,
            params: [a.projectIds],
        };
    }
    return {
    ...
    try {
        ...
        const { rows } = await pool.query(
            `SELECT p.id, p.project_code, p.title, p.rag_status, p.completion_pct, p.next_review_at, p.last_update_at,
                    p.theme_name, p.team_id, p.artefact_id, p.artefact_title, p.project_phase, p.semester, p.academic_year,
...
async function listDepartments(req, res, next) {
    try {
        ...
        const params = [];
        if (!global) {
            params.push(a.instituteIds);
            params.push(a.departmentIds);
            deptCondition = `WHERE (d.institute_id = ANY($1::uuid[]) OR d.id = ANY($2::int[])) AND d.is_active = TRUE`;
        } else {
            deptCondition = `WHERE d.is_active = TRUE`;
        }

        const { rows } = await pool.query(
            `SELECT d.id, d.code, d.name, d.institute_id, i.name AS institute_name,
             ...
             LEFT JOIN projects p ON p.department_id = d.id AND p.is_active = TRUE
             ${deptCondition}
             GROUP BY d.id, i.name, hu.id
             ORDER BY d.name`,
            params
        );
...
module.exports = { listProjects, listFilterOptions, listIssues, listReviews, listDepartments };
